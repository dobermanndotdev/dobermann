...
func (r *accountR) GetTeams() TeamSlice {
	if r == nil {
		return nil
	}
	return r.Teams
...
var (
	accountAllColumns            = []string{"id", "name", "created_at"}
	accountColumnsWithoutDefault = []string{"id", "name"}
	accountColumnsWithDefault    = []string{"created_at"}
...
func (o *Account) Teams(mods ...qm.QueryMod) teamQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"teams\".\"account_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (accountL) LoadTeams(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	var slice []*Account
	var object *Account
	...
	query := NewQuery(
		qm.From(`teams`),
		qm.WhereIn(`teams.account_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Account appropriately.
func (o *Account) AddTeams(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Team) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.AccountID = o.ID
